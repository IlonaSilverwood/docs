...
- Write a short paper in Markdown format using `paper.md` as file name, including a title, summary, author names, affiliations, and key references using APA citations. See our [example paper](#example-paper-and-bibliography) to follow the correct format.
...
```eval_rst
.. important:: Using an ISSN in the ISBN field will result in an error during the acceptance process.
```
